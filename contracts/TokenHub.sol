...
contract TokenHub is ITokenHub, System, IParamSubscriber, IApplication, ISystemReward {
  ...
  uint8 constant public   MINIMUM_BEP40_SYMBOL_LEN = 3;
  uint8 constant public   MAXIMUM_BEP40_SYMBOL_LEN = 8;
  ...
  uint256 constant public MAX_GAS_FOR_CALLING_BEP40=50000;
  ...
  mapping(address => uint256) public bep40ContractDecimals;
  mapping(address => bytes32) private contractAddrToBEP2Symbol;
  mapping(bytes32 => address) private bep2SymbolToContractAddr;

  event transferInSuccess(address bep40Addr, address refundAddr, uint256 amount);
  event transferOutSuccess(address bep40Addr, address senderAddr, uint256 amount, uint256 relayFee);
  event refundSuccess(address bep40Addr, address refundAddr, uint256 amount, uint32 status);
  event refundFailure(address bep40Addr, address refundAddr, uint256 amount, uint32 status);
  ...
  function init() onlyNotInit external {
    ...
    bep40ContractDecimals[address(0x0)] = 18; // BNB decimals is 18
    alreadyInit=true;
  }
  ...
  function handleTransferInSynPackage(bytes memory msgBytes) internal returns(bytes memory) {
    ...
    if (resCode != TRANSFER_IN_SUCCESS) {
      uint256 bep2Amount = convertToBep2Amount(transInSynPkg.amount, bep40ContractDecimals[transInSynPkg.contractAddr]);
  ...
  function doTransferIn(TransferInSynPackage memory transInSynPkg) internal returns (uint32) {
    if (transInSynPkg.contractAddr==address(0x0)) {
      ...
      uint256 actualBalance = IBEP40(transInSynPkg.contractAddr).balanceOf{gas: MAX_GAS_FOR_CALLING_BEP40}(address(this));
      if (actualBalance < transInSynPkg.amount) {
        return TRANSFER_IN_FAILURE_INSUFFICIENT_BALANCE;
      }
      bool success = IBEP40(transInSynPkg.contractAddr).transfer{gas: MAX_GAS_FOR_CALLING_BEP40}(transInSynPkg.recipient, transInSynPkg.amount);
      if (success) {
        emit transferInSuccess(transInSynPkg.contractAddr, transInSynPkg.recipient, transInSynPkg.amount);
        return TRANSFER_IN_SUCCESS;
      } else {
  ...
  function doRefund(TransferOutAckPackage memory transOutAckPkg) internal {
    if (transOutAckPkg.contractAddr==address(0x0)) {
      ...
      for (uint256 index = 0; index<transOutAckPkg.refundAmounts.length; index++) {
        bool success = IBEP40(transOutAckPkg.contractAddr).transfer{gas: MAX_GAS_FOR_CALLING_BEP40}(transOutAckPkg.refundAddrs[index], transOutAckPkg.refundAmounts[index]);
        if (success) {
          emit refundSuccess(transOutAckPkg.contractAddr, transOutAckPkg.refundAddrs[index], transOutAckPkg.refundAmounts[index], transOutAckPkg.status);
        } else {
          emit refundFailure(transOutAckPkg.contractAddr, transOutAckPkg.refundAddrs[index], transOutAckPkg.refundAmounts[index], transOutAckPkg.status);
  ...
  function handleTransferOutFailAckPackage(bytes memory msgBytes) internal {
    ...
    uint256 bep40TokenDecimals = bep40ContractDecimals[transOutSynPkg.contractAddr];
    for (uint idx=0;idx<transOutSynPkg.amounts.length;idx++) {
      transOutSynPkg.amounts[idx] = convertFromBep2Amount(transOutSynPkg.amounts[idx], bep40TokenDecimals);
    }
  ...
  function transferOut(address contractAddr, address recipient, uint256 amount, uint64 expireTime) external override onlyInit payable returns (bool) {
    ...
    if (contractAddr==address(0x0)) {
      ...
      uint256 bep40TokenDecimals=bep40ContractDecimals[contractAddr];
      require(bep40TokenDecimals<=BEP2_TOKEN_DECIMALS || (bep40TokenDecimals>BEP2_TOKEN_DECIMALS && amount.mod(10**(bep40TokenDecimals-BEP2_TOKEN_DECIMALS))==0), "invalid transfer amount: precision loss in amount conversion");
      convertedAmount = convertToBep2Amount(amount, bep40TokenDecimals);// convert to bep2 amount
      if (isMiniBEP2Token(bep2TokenSymbol)) {
        require(convertedAmount >= 1e8 , "For miniToken, the transfer amount must not be less than 1");
      }
      require(bep40TokenDecimals>=BEP2_TOKEN_DECIMALS || (bep40TokenDecimals<BEP2_TOKEN_DECIMALS && convertedAmount>amount), "amount is too large, uint256 overflow");
      require(convertedAmount<=MAX_BEP2_TOTAL_SUPPLY, "amount is too large, exceed maximum bep2 token amount");
      require(IBEP40(contractAddr).transferFrom(msg.sender, address(this), amount));
    }
  ...
  function bindToken(bytes32 bep2Symbol, address contractAddr, uint256 decimals) external override onlyTokenManager {
    ...
    bep40ContractDecimals[contractAddr] = decimals;
  }
  ...
  function isMiniBEP2Token(bytes32 symbol) internal pure returns(bool) {
     ...
     if (symbolLength < MINIMUM_BEP40_SYMBOL_LEN + 5) {
       return false;
     }
  ...
  function convertToBep2Amount(uint256 amount, uint256 bep40TokenDecimals) internal pure returns (uint256) {
    if (bep40TokenDecimals > BEP2_TOKEN_DECIMALS) {
      return amount.div(10**(bep40TokenDecimals-BEP2_TOKEN_DECIMALS));
    }
    return amount.mul(10**(BEP2_TOKEN_DECIMALS-bep40TokenDecimals));
  }

  function convertFromBep2Amount(uint256 amount, uint256 bep40TokenDecimals) internal pure returns (uint256) {
    if (bep40TokenDecimals > BEP2_TOKEN_DECIMALS) {
      return amount.mul(10**(bep40TokenDecimals-BEP2_TOKEN_DECIMALS));
    }
    return amount.div(10**(BEP2_TOKEN_DECIMALS-bep40TokenDecimals));
  }
