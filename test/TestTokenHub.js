...
function buildBindPackage(bindType, bep2TokenSymbol, bep40Addr, totalSupply, peggyAmount, decimals) {
...
function buildTransferInPackage(bep2TokenSymbol, bep40Addr, amount, recipient, refundAddr) {
    let timestamp = Math.floor(Date.now() / 1000); // counted by second
    let initialExpireTimeStr = (timestamp + 3).toString(16); // expire at 5 second later
    const initialExpireTimeStrLength = initialExpireTimeStr.length;
    let expireTimeStr = initialExpireTimeStr;
...
function buildTransferInPackageWithRelayFee(bep2TokenSymbol, bep40Addr, amount, recipient, refundAddr, syncRelayFee) {
    let timestamp = Math.floor(Date.now() / 1000); // counted by second
    let initialExpireTimeStr = (timestamp + 3).toString(16); // expire at 5 second later
    const initialExpireTimeStrLength = initialExpireTimeStr.length;
    let expireTimeStr = initialExpireTimeStr;
...
contract('TokenHub', (accounts) => {
    ...
    it('Relay expired bind package', async () => {
        ...
        try {
            ...
            assert.ok(error.toString().includes("only bep40 owner can approve this bind request"));
        }
    ...
    it('Reject bind', async () => {
        ...
        try {
            ...
            assert.ok(error.toString().includes("only bep40 owner can reject"));
        }
    ...
    it('Transfer from BSC to BC', async () => {
        ...
        try {
            ...
            assert.ok(error.toString().includes("BEP40: transfer amount exceeds allowance"));
        }
        ...
        truffleAssert.eventEmitted(tx, "transferOutSuccess",(ev) => {
            return ev.amount.eq(web3.utils.toBN(amount)) && ev.bep40Addr.toString().toLowerCase() === abcToken.address.toLowerCase();
        });
    ...
    it('Relay refund package', async () => {
        ...
        const packageBytes = RLP.encode([
            abcToken.address,           //bep40 contract address
            [1e18],                    //amount
    ...
    it('Batch transfer out', async () => {
        ...
        truffleAssert.eventEmitted(tx, "transferOutSuccess",(ev) => {
            return ev.amount.eq(web3.utils.toBN(3e16)) && ev.bep40Addr.toString().toLowerCase() === "0x0000000000000000000000000000000000000000";
        });
    ...
    it('Bind malicious BEP40 token', async () => {
    ...
    it('bind and transfer miniToken', async () => {
        ...
        truffleAssert.eventEmitted(tx, "transferOutSuccess",(ev) => {
            return ev.amount.eq(web3.utils.toBN(amount)) && ev.bep40Addr.toString().toLowerCase() === miniToken.address.toLowerCase();
        });
